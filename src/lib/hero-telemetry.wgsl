...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let evergreen = vec3f(0.098039, 0.227451, 0.160784);
  let foundry = vec3f(0.658824, 0.572549, 0.486275);
  let leftClear = smoothstep(0.34, 0.62, uv.x);
  var a = traces * 0.32 + spark * 0.08;
  a *= 0.55 * leftClear;
  a = clamp(a, 0.0, 0.34);
  let col = mix(evergreen, foundry, spark * 0.45);
